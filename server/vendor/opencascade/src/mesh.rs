...
impl Mesher {
    pub fn new(shape: &Shape) -> Self {
        Self::new_with_deflection(shape, 0.01)
    }

    pub fn new_with_deflection(shape: &Shape, deflection: f64) -> Self {
        let inner = ffi::BRepMesh_IncrementalMesh_ctor(&shape.inner, deflection);
