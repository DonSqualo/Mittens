...
fn load_stl_mesh(path: &Path) -> Result<MeshData> {
    ...
    out.colors.resize(out.positions.len(), 1.0);
    Ok(out)
}
...
fn load_step_mesh(path: &Path) -> Result<MeshData> {
    ...
    // STEP meshing can explode in triangle count on large assemblies.
    // Default is tuned for interactive viewing; override via env.
    let default_deflection = std::fs::metadata(path)
        .ok()
        .map(|m| {
            // Heuristic: very large STEP files can explode in triangle count and memory usage.
            // Prefer a coarse, interactive-safe default; allow override via MITTENS_STEP_DEFLECTION.
            let sz = m.len();
            if sz > 150 * 1024 * 1024 {
                30.0
        ...
        .unwrap_or(0.05);
    let deflection = std::env::var("MITTENS_STEP_DEFLECTION")
        .ok()
        .and_then(|v| v.parse::<f64>().ok())
        .unwrap_or(default_deflection);
    let mesh = shape.mesh_with_deflection(deflection);
