...
/// Extract acoustic field data generated by Lua stdlib.acoustics module
/// Converts Lua table format to binary FieldData format for renderer
fn try_extract_lua_acoustic_field(field_table: &mlua::Table) -> Option<field::FieldData> {
    let grid_x_points: usize = field_table.get("grid_x_points").ok()?;
    let grid_z_points: usize = field_table.get("grid_z_points").ok()?;
    let frequency: f64 = field_table.get("frequency").ok()?;
    let amplitude: f64 = field_table.get("amplitude").unwrap_or(100.0);
    let max_pressure: f64 = field_table.get("max_pressure").unwrap_or(1.0);

    // Get bounds (scalar values, not tables)
    let x_min: f64 = field_table.get("x_min").ok()?;
    let x_max: f64 = field_table.get("x_max").ok()?;
    let z_min: f64 = field_table.get("z_min").ok()?;
    let z_max: f64 = field_table.get("z_max").ok()?;

    // Get pressure data (flattened 1D array)
    let pressure_data: mlua::Table = field_table.get("pressure_data").ok()?;

    let mut magnitude = Vec::with_capacity(grid_x_points * grid_z_points);
    for i in 1..=(grid_x_points * grid_z_points) {
        let val: f64 = pressure_data.get(i).unwrap_or(0.0);
        magnitude.push(val as f32);
    }

    // Create FieldData with acoustic-specific colormap (Red = high pressure, Blue = low pressure)
    info!(
        "Extracted Lua acoustic field: {}x{} grid, f={:.4}Hz, max_p={:.1}Pa, bounds X=[{:.0},{:.0}]mm Z=[{:.0},{:.0}]mm",
        grid_x_points, grid_z_points, frequency, max_pressure, x_min, x_max, z_min, z_max
    );

    Some(field::FieldData {
        plane_type: field::PlaneType::XZ,
        colormap: field::Colormap::PressureBlueRed,  // Red = high, Blue = low
        slice_width: grid_x_points,
        slice_height: grid_z_points,
        slice_bounds: [x_min, x_max, z_min, z_max],
        slice_offset: 0.0,  // XZ plane at Y=0 (center)
        slice_bx: Vec::new(),  // Not used for acoustic field
        slice_bz: Vec::new(),  // Not used for acoustic field
        slice_magnitude: magnitude,
...
fn try_compute_acoustic_field(lua: &mlua::Lua, content: &str) -> Option<field::FieldData> {
    ...
    let has_acoustic = content.contains("AcousticField") || content.contains("Acoustics")
    ...
    if !has_acoustic {
        eprintln!("DEBUG: No acoustic keywords found in content, skipping");
        return None;
    }
    
    eprintln!("DEBUG: Acoustic keywords found, attempting to extract field");
    
    // Note: Lua code has already been executed by process_single_file,
    // so globals are already set up. Just read them without re-executing.
    let globals = lua.globals();

    // Stage 11: Try to get AcousticField generated by stdlib.acoustics
    eprintln!("DEBUG: Checking for AcousticField table in Lua globals");
    match globals.get::<_, mlua::Table>("AcousticField") {
        Ok(field_table) => {
            eprintln!("DEBUG: Found AcousticField table, extracting...");
            return try_extract_lua_acoustic_field(&field_table);
        }
        Err(e) => {
            eprintln!("DEBUG: AcousticField not found or not a table: {}", e);
        }
    }

    // Fallback: Try to get Acoustic config (old style)
