...
pub fn generate_external_thread(
    ...
    let num_verts = total_rings * ring_verts + 2;
    ...
    for zi in 0..z_segments {
        ...
        for ai in 0..angle_segments {
            ...
            tri_verts.extend_from_slice(&[ring_base + a0, ring_base + a1, ring_next + a1]);
            tri_verts.extend_from_slice(&[ring_base + a0, ring_next + a1, ring_next + a0]);
        }
    }
...
pub fn generate_internal_thread(
    ...
    // Vertices: inner ring + outer ring at each z level
    let ring_verts = angle_segments * 2;
    let total_rings = z_segments + 1;
    ...
    for zi in 0..=z_segments {
        ...
        for ai in 0..angle_segments {
            ...
            // For internal thread the thread sticks inward.
            let phase = (angle / pi2 - z_phase).rem_euclid(1.0);
            let profile_val = thread_profile(phase);
            ...
            vert_props.extend_from_slice(&[x as f32, y as f32, z as f32, nx as f32, ny as f32, 0.0]);
        }
    ...
    for zi in 0..z_segments {
        ...
        for ai in 0..angle_segments {
            ...
            tri_verts.extend_from_slice(&[ring_base + a0, ring_next + a0, ring_next + a1]);
            tri_verts.extend_from_slice(&[ring_base + a0, ring_next + a1, ring_base + a1]);
        }
    }
    ...
    for ai in 0..angle_segments {
        ...
        tri_verts.extend_from_slice(&[bottom_inner + a0, bottom_inner + a1, bottom_outer + a1]);
        tri_verts.extend_from_slice(&[bottom_inner + a0, bottom_outer + a1, bottom_outer + a0]);
    }

    // Top cap annulus (normal +Z)
    ...
    for ai in 0..angle_segments {
        ...
        tri_verts.extend_from_slice(&[top_inner + a1, top_inner + a0, top_outer + a0]);
        tri_verts.extend_from_slice(&[top_inner + a1, top_outer + a0, top_outer + a1]);
    }
