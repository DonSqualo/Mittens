...
local bath_outer =
	box(Bath.length + 2 * Bath.wall, Bath.width + 2 * Bath.wall, Bath.depth + Bath.wall):center(true, true, false)

local bath_inner = box(Bath.length, Bath.width, Bath.depth):center(true, true, false):at(0, 0, Bath.wall)

local bath_shell = difference(bath_outer, bath_inner):material(aluminum):color(0.7, 0.7, 0.75, 1.0):tag("bath_shell")
...
local frame = Extrusions.structural_frame(Bath.length + 200, Bath.width + 200, Bath.depth - 50, "20x20")
...
local acoustic_field = Acoustics.create_default_field(Bath.length, Simulation.frequency, Simulation.amplitude)
...
-- Add main trunk edge
local trunk_length = math.sqrt((trunk_x_end - trunk_x_start) ^ 2 + 0 + 0)
...
local function count_primitives(obj, name)
	...
	if obj._type == "group" or obj._type == "assembly" then
		...
		print(
			string.format("  Group '%s': %d children -> %d primitives", obj._name or "?", #(obj._children or {}), count)
		)
		return count
	...
	elseif obj._type == "shape" then
		print(
			string.format(
				"  Primitive: %s (%s)",
				obj._tag or obj._name or "?",
				obj._metadata and obj._metadata.primitive or "?"
			)
		)
		return 1
	else
