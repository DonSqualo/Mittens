...
function Threads.external(params)
  ...
  local clearance = params.clearance or 0  -- 0.2mm recommended for 3D printing
  
  if not major_diameter or not pitch then
    error("external_thread requires major_diameter and pitch")
  end
  
  -- Create raw manifold primitive
  local shape = {
    _type = "shape",
    _metadata = {
...
function Threads.intermediate_ring(params)
  ...
  -- CRITICAL FIX (Iteration 5): Thread clearance for proper mesh
  -- Internal bore must be LARGER than external major for threads to mesh
  -- Standard metric thread clearance: 0.25-0.3mm for H7/g6 fit
  local thread_clearance = 0.25  -- mm (allows male thread to fit into female bore)
  local inner_diameter = outer_diameter + thread_clearance
  
  -- Create external thread on outside
  ...
  -- Create internal thread on inside (at opposite orientation)
  ...
  -- Create a ring shape by unioning the thread parts
  -- Return as a composite group
  local ring = {
    _type = "shape",
    _metadata = {
      primitive = "group",
